...
class VisionDataset(object):
    ...
    def get_loader(self, indices, transforms, train, shuffle=True, target_transforms=None):
        sampler = None
        if indices is not None: sampler = SubsetRandomSampler(indices) if (
                shuffle and train) else SubsetSequentialSampler(indices)
        ...
        if self.opt.dataset in ['CIFAR10', 'CIFAR100', 'MNIST', 'KMNIST', 'FashionMNIST']:
            return DataLoader(
                getattr(torchvision.datasets, self.opt.dataset)(root=self.opt.data_dir, train=train, download=True,
                                                                transform=transforms,
                                                                target_transform=target_transforms), sampler=sampler,
                **self.kwargs)
        elif self.opt.dataset == 'SVHN':
            split = 'train' if train else 'test'
            return DataLoader(
                getattr(torchvision.datasets, self.opt.dataset)(root=self.opt.data_dir, split=split, download=True,
                                                                transform=transforms,
                                                                target_transform=target_transforms), sampler=sampler,
                **self.kwargs)
        else:
            ...
            return DataLoader(
                torchvision.datasets.ImageFolder(self.opt.data_dir + '/' + self.opt.dataset + '/' + subfolder,
    ...
    def gen_cl_mapping(self):
        ...
        if self.opt.dataset == 'SVHN':
            train_class_labels_dict, test_class_labels_dict = classwise_split(
        ...
        assert (
                self.opt.num_tasks * self.opt.num_classes_per_task <= self.opt.total_num_classes), "num_classes lesser than classes_per_task * num_tasks"
        ...
        if self.opt.num_pretrain_classes > 0:
            pretrain_target_transform = ReorderTargets(
                pretrain_class_list)  # Uses target_transforms to remap the class order according to class list
            assert (len(
                pretrain_class_list) == self.opt.num_pretrain_classes), "Error in generating the pretraining list"
            ...
            self.pretrain_loader = self.get_loader(indices=pretrainidx, transforms=self.train_transforms, train=True,
                                                   target_transforms=pretrain_target_transform)
            self.pretest_loader = self.get_loader(indices=pretestidx, transforms=self.test_transforms, train=False,
                                                  target_transforms=pretrain_target_transform)

        self.class_mask = torch.from_numpy(np.kron(np.eye(self.opt.num_tasks, dtype=int), np.ones((
            self.opt.num_classes_per_task,
            self.opt.num_classes_per_task)))).cuda()  # Generates equal num_classes for all tasks.
        continual_target_transform = ReorderTargets(
            cl_class_list)  # Remaps the class order to a 0-n order, required for crossentropy loss using class list
        trainidx, testidx = [], []
        ...
        for cl in cl_class_list:  # Selects classes from the continual learning list and loads memory and test indices, which are then passed to a subset sampler
            ...
            trainidx += train_class_labels_dict[cl][
        ...
        self.cltrain_loader = self.get_loader(indices=trainidx, transforms=self.train_transforms, train=True,
                                              target_transforms=continual_target_transform)
        self.cltest_loader = self.get_loader(indices=testidx, transforms=self.test_transforms, train=False,
                                             target_transforms=continual_target_transform)
